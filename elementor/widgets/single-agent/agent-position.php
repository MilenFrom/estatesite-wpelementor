...
class Houzez_Agent_Position extends Widget_Base {
	...
	protected function render() {
		...
		$agent_position = get_post_meta( get_the_ID(), 'fave_agent_position', true ) /* TODO unmapped fave_ key: fave_agent_position */;
		$agent_company = get_post_meta( get_the_ID(), 'fave_agent_company', true ) /* TODO unmapped fave_ key: fave_agent_company */;
		$agent_agency_id = get_post_meta( get_the_ID(), 'fave_agent_agencies', true ) /* TODO unmapped fave_ key: fave_agent_agencies */;
		...
		?>
		<p class="agent-list-position mb-1"> <?php echo esc_attr($agent_position); ?>
			<?php if(!empty($agent_company)) { ?>
				
				<span itemprop="jobTitle"><?php echo $settings['position_text']; ?></span>
				<a<?php echo $href; ?>>
