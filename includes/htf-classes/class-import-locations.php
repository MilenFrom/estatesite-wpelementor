...
class Houzez_Import_Locations {
     ...
     *
     * Uses path-based comparison to handle www/non-www and scheme variations
     */
    private static function url_to_path($url) {
        ...
        // Parse URLs to extract paths for comparison (handles www/non-www and scheme variations)
        $url_parts = wp_parse_url($url);
        $uploads_url_parts = wp_parse_url($uploads_url);

        // Ensure we have valid path components
        if (empty($url_parts['path']) || empty($uploads_url_parts['path'])) {
            return false;
        }

        $url_path = $url_parts['path'];
        $uploads_url_path = $uploads_url_parts['path'];

        // SECURITY: Check if the URL path starts with the uploads URL path
        if (strpos($url_path, $uploads_url_path) !== 0) {
            return false; // URL must be within uploads directory
        }

        // Extract the relative path after the uploads base path
        $relative_path = substr($url_path, strlen($uploads_url_path));
        ...
        if ($real_uploads_path === false || $real_file_dir === false) {
            return false; // Invalid paths
        }

        $real_file_path = $real_file_dir . '/' . basename($file_path);
    ...
    public static function get_locations_csv_headers() {
        ...
        // Fallback: Try remote fetch if local mapping fails
        // SECURITY: Only allow URLs from the same site
        $site_url = wp_parse_url(home_url(), PHP_URL_HOST);
        $file_url_host = wp_parse_url($file_url, PHP_URL_HOST);

        // Allow www and non-www variants of the same domain
        $site_url_clean = preg_replace('/^www\./', '', $site_url);
        $file_url_host_clean = preg_replace('/^www\./', '', $file_url_host);

        if ($site_url_clean !== $file_url_host_clean) {
            wp_send_json_error('File not found or access denied');
            return;
        }

        // SECURITY: Verify URL is for a CSV file
        $file_extension = strtolower(pathinfo(wp_parse_url($file_url, PHP_URL_PATH), PATHINFO_EXTENSION));
        if ($file_extension !== 'csv') {
            wp_send_json_error('Invalid file type');
            return;
        }

        // Fetch headers remotely
        $headers = self::get_csv_headers_remote($file_url);
        if (!empty($headers)) {
            wp_send_json_success($headers);
        } else {
            wp_send_json_error('Unable to read CSV headers. Please check file format or URL.');
        }
    }

    /**
     * Get CSV headers from a remote URL (fallback method)
     * Only reads the first line to minimize data transfer
     */
    private static function get_csv_headers_remote($url) {
        $response = wp_remote_get($url, array(
            'timeout' => 15,
            'sslverify' => false,
            'headers' => array(
                'Range' => 'bytes=0-4096' // Only fetch first 4KB for headers
            )
        ));
        ...
        // Parse the first line as CSV headers
        $lines = explode("\n", $body);
        if (empty($lines[0])) {
            return array();
        }
