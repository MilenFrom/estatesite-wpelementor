...
class Houzez_Post_Type_Property {
    ...
    public static function custom_columns_manage( $column ) {
        ...
        switch ($column)
        {
            ...
            case 'houzez_actions':
                ...
                $fave_featured = \EstateSite\Core\Property::get( $post->ID, 'featured' );
    ...
    public static function save_property_post_type($meta_id, $property_id, $meta_key, $meta_value) {
        ...
        $relevant_keys = ['fave_featured', \EstateSite\Core\Property::key( 'id' ), 'fave_property_location', \EstateSite\Core\Property::key( 'currency' ), 'houzez_geolocation_lat', 'houzez_geolocation_long'];
        ...
        if ( 'fave_property_id' === $meta_key ) {
            if( ! empty( houzez_option('auto_property_id', 0) ) ) {
                $existing_id     = \EstateSite\Core\Property::get( $property_id, 'id' );
                $pattern = houzez_option( 'property_id_pattern' );
                $new_id   = preg_replace( '/{ID}/', $property_id, $pattern );
                
                if ( $existing_id !== $new_id ) {
                    \EstateSite\Core\Property::set( $property_id, 'id', $new_id );
                }
            }
        }
        ...
        if ( 'houzez_geolocation_lat' !== $meta_key || 'houzez_geolocation_long' !== $meta_key ) {
            $lat_long = \EstateSite\Core\Property::get( $property_id, 'location' );
        ...
        if(class_exists('Houzez_Currencies')) {
            ...
            if ( 'fave_currency' === $meta_key ) {
                $currency_code = \EstateSite\Core\Property::get( $property_id, 'currency' );
                $currencies = Houzez_Currencies::get_property_currency_2($property_id, $currency_code);

                update_post_meta( $property_id, 'fave_currency_info', $currencies ) /* TODO unmapped fave_ key: fave_currency_info */;
            }
    ...
    public static function houzez_mark_featured() {
        ...
        \EstateSite\Core\Property::set( $post_id, 'featured', 1 );
    ...
    public static function duplicate_property_post_as_draft() {
        ...
        if (isset( $post ) && $post != null) {
            ...
            \EstateSite\Core\Property::set( $new_post_id, 'featured', 0 );
            update_post_meta( $new_post_id, 'houzez_featured_listing_date', '' );
            \EstateSite\Core\Property::set( $new_post_id, 'payment_status', 'not_paid' );

            if( houzez_option('auto_property_id', 0) != 0 ) {
                $pattern = houzez_option( 'property_id_pattern' );
                $new_id   = preg_replace( '/{ID}/', $new_post_id, $pattern );
                \EstateSite\Core\Property::set( $new_post_id, 'id', $new_id );
            }
    ...
    public static function houzez_property_admin_custom_query($query) {
        ...
        if ( $pagenow == 'edit.php' && $post_type == 'property' ) {
            ...
            if ( isset( $_GET['property_id'] ) && ! empty( $_GET['property_id'] ) ) {
                ...
                $meta_query[] = array(
                    'key' => \EstateSite\Core\Property::key( 'id' ),
            ...
            if ( 'price' == $orderby ) {
                ...
                $price_query = array(
                    'relation' => 'OR',
                    'price_clause' => array(
                        'key' => \EstateSite\Core\Property::key( 'price' ),
                        'compare' => 'EXISTS',
                        'type' => 'NUMERIC'
                    ),
