...
/**
 * Map an fts_template_type to the post type whose single page makes the most
 * sense to use as a preview target.
 *
 * Used by the editor "Preview Settings" panel:
 *   - For layout templates (header/footer/before/after/megamenu/custom_block)
 *     there's no canonical post type — let the user pick from any public CPT.
 *   - For `single-*` templates the target is fixed (single-listing → property,
 *     single-agent → houzez_agent, etc.) since the template is meant to render
 *     INSIDE that post type's single view.
 *
 * Returns:
 *   - string $post_type     — forced preview post type (single-* templates), or
 *   - null                   — let the user pick freely (layout templates), or
 *   - false                  — template type unknown / not an fts_builder doc.
 *
 * @param string $template_type fts_template_type meta value (e.g. 'tmp_header').
 * @return string|null|false
 */
function houzez_tb_preview_post_type_for( $template_type ) {
    switch ( $template_type ) {
        ...
        case 'single-agent':
            return 'houzez_agent';
        case 'single-agency':
            return 'houzez_agency';
        case 'single-post':
            return 'post';

        case 'tmp_header':
        case 'tmp_footer':
        case 'tmp_before_header':
        case 'tmp_after_header':
        case 'tmp_before_footer':
        case 'tmp_after_footer':
        case 'tmp_megamenu':
        case 'tmp_custom_block':
            return null; // user picks any public post type
